...
def one_plot( alg : Alg, bf : BoundedFunction):
    # Initial setup
...
def two_plot(alg1 : Alg, alg2 : Alg, bf : BoundedFunction):
    # Initial setup
    x = np.linspace(bf.range_x1[0], bf.range_x1[1])
    y = np.linspace(bf.range_x1[0], bf.range_x2[1])
    X, Y = np.meshgrid(x, y)
    ...
    fig, ax = plt.subplots(figsize=(10, 10))
    contour = ax.contourf(X,Y,Z, levels=10, cmap = "plasma")
    scatter1 = ax.scatter([], [] , color='r', s = 10, zorder = 10)
    scatter2 = ax.scatter([], [], color = 'b', s = 10, zorder = 10)
    ax.set_title(f"Interactive Plot of Algorithm {alg1.name} vs {alg2.name} with Function {bf.name}")
    ax.set_xlabel('X-axis')
    ax.set_ylabel('Y-axis')
    colorbar = plt.colorbar(contour, ax=ax)
    colorbar.set_label('Function Value')
    ...
    # Function to update scatter data
    def update_scatter():
        for alg, scatter in zip([alg1, alg2], [scatter1, scatter2]):   
            alg.update_points(bf)
            points = alg.get_points()
            scatter_data = np.array([[p[0] for p in points], [p[1] for p in points]])
            scatter.set_offsets(scatter_data.T)
